...
/// How a chosen background image fills the frame.
...
thread_local! {
    // Deliberately NOT in invalidate_baked — the user's image survives theme switches.
    static BG_IMG: RefCell<Option<Texture2D>> = const { RefCell::new(None) };
    static BG_FIT: Cell<BgFit> = const { Cell::new(BgFit::Cover) };
    static BG_DIM: Cell<f32> = const { Cell::new(0.3) };
}

/// Install (or clear, with `None`) the user background image.
pub fn set_background(tex: Option<Texture2D>) {
    BG_IMG.with(|c| *c.borrow_mut() = tex);
}
pub fn has_background() -> bool {
    BG_IMG.with(|c| c.borrow().is_some())
}
pub fn set_bg_fit(f: BgFit) {
    BG_FIT.with(|c| c.set(f));
}
pub fn bg_fit() -> BgFit {
    BG_FIT.with(|c| c.get())
}
pub fn set_bg_dim(d: f32) {
    BG_DIM.with(|c| c.set(d.clamp(0.0, 0.95)));
}
pub fn bg_dim() -> f32 {
    BG_DIM.with(|c| c.get())
}

/// Draw the background image fullscreen (cover / contain / stretch) with a dim
/// overlay toward ink, at `alpha` (1 = full opaque draw; < 1 = a feedback-decay
/// blend so trails settle over the image). Returns false if no image is set.
/// Call under `view::apply_screen_camera()` (screen space; the image is upright,
/// so no flip is needed unlike render-target textures).
pub fn draw_background(alpha: f32) -> bool {
    BG_IMG.with(|c| {
        let b = c.borrow();
        let Some(tex) = b.as_ref() else {
            return false;
        };
        let a = alpha.clamp(0.0, 1.0);
        let (sw, sh) = (view::screen_w(), view::screen_h());
        let (tw, th) = (tex.width().max(1.0), tex.height().max(1.0));
        let fit = bg_fit();
        let (dw, dh) = match fit {
            BgFit::Stretch => (sw, sh),
            BgFit::Cover => {
        ...
        let (ox, oy) = ((sw - dw) * 0.5, (sh - dh) * 0.5);
        if fit == BgFit::Contain && a >= 0.999 {
            clear_background(ink()); // fill the letterbox on a full draw
        }
        draw_texture_ex(tex, ox, oy, with_alpha(WHITE, a), DrawTextureParams { dest_size: Some(vec2(dw, dh)), ..Default::default() });
        let dim = bg_dim();
        if dim > 0.0 {
            draw_rectangle(0.0, 0.0, sw, sh, with_alpha(ink(), dim * a));
        }
        true
    })
}
