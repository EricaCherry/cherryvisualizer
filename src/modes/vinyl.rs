...
impl Mode for Vinyl {
    ...
    fn update(&mut self, ctx: &FrameCtx) {
        let dt = ctx.dt;
        // rms is already smoothed once upstream (analysis), so read it directly.
        self.angle += dt * std::f32::consts::TAU * (self.rpm / 60.0) * (0.6 + ctx.feat.rms * 0.8);
    ...
    fn draw(&self, ctx: &FrameCtx) {
        ...
        for r in 0..rings {
            ...
            let energy = 0.18 + if fr > 0.5 { ctx.feat.treble } else { ctx.feat.bass };
            let c = with_alpha(grade(energy), 0.5);
            let mut prev: Option<(f32, f32)> = None;
            for s in 0..=segs {
                let a = s as f32 / segs as f32 * std::f32::consts::TAU + self.angle;
                let wi = (s + r * 5) % wave.len();
                let rr = base_r + wave[wi] * norm * self.groove * 0.22 * (0.6 + ctx.feat.rms * 0.8);
